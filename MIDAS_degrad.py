...
def MIDAS_degrad(args):
    ...
    for image_config in yaml_list:
        ...
        image_stego = load_image(args.save_path + '/' + image_name + f"_hide_pw_{args.seed}.png").resize((512, 512))
        image_hide_latent = ode.image2latent(image_stego, ref_vae=args.ref_model)
        
        ref_image = load_image(args.save_path + '/' + image_name + f"_ref_pw_{args.seed}.png").resize((512, 512))
        ...
        for i in range(args.num_secret_image):
            if args.key_type == 'noise_flip':
                noise_mask_p.append(torch.randint(low=0, high=int(1/args.personal_key_strength), size=(1, 4, 64, 32), device=image_hide_latent.device,
                                    generator=torch.Generator(device=image_hide_latent.device).manual_seed(args.seed + 1 + i),
                                    dtype=torch.float16,requires_grad=False))
            elif args.key_type == 'random_basis':
                B, C, H_p, W_p = 1, 4, 64, 32
                D_p = C * H_p * W_p
                d_p = int(D_p * args.personal_key_strength)

                g_p.append(torch.Generator(device=image_hide_latent.device).manual_seed(args.seed + 1 + i))
                all_indices = torch.randperm(D_p, generator=g_p[-1], device=image_hide_latent.device)
                selected_indices_p.append(all_indices[:d_p].detach())

                P_d , _ = torch.linalg.qr(torch.randn(d_p, d_p, generator=g_p[-1], device=image_hide_latent.device))
                P_d = P_d.to(torch.float16)
                P_d_p.append(P_d)

        if args.key_type == 'noise_flip':
            # mask flip
            noise_mask = torch.randint(low=0, high=int(1/args.key_strength), size=image_hide_latent.shape, device=image_hide_latent.device,
                                generator=torch.Generator(device=image_hide_latent.device).manual_seed(args.seed),
                                dtype=torch.float16,requires_grad=False)
        elif args.key_type == 'random_basis':
            B, C, H, W = image_hide_latent.shape
            D = C * H * W
            d = int(D * args.key_strength)
            g = torch.Generator(device=image_hide_latent.device).manual_seed(args.seed)
            all_indices = torch.randperm(D, generator=g, device=image_hide_latent.device)
            selected_indices = all_indices[:d].detach()
            Q_d, _ = torch.linalg.qr(torch.randn(d, d, generator=g, device=image_hide_latent.device))
            Q_d = Q_d.to(torch.float16)
        
        for j in range(4):
            if j == 0:
                image_stego_degrad = image_stego
            elif j == 1:
                quality = 70
                bgr = cv2.cvtColor(np.array(image_stego), cv2.COLOR_RGB2BGR)
                encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), quality]
                _, compressed = cv2.imencode('.jpg', bgr, encode_param)
                image_stego_degrad = cv2.imdecode(compressed, cv2.IMREAD_COLOR)
                image_stego_degrad = Image.fromarray(cv2.cvtColor(image_stego_degrad, cv2.COLOR_BGR2RGB))
            elif j == 2:  # Gaussian noise
                img_np = np.array(image_stego).astype(np.float32)
                sigma_noise = 5 
                noise = np.random.normal(0, sigma_noise, img_np.shape).astype(np.float32)
                noisy = np.clip(img_np + noise, 0, 255).astype(np.uint8)
                image_stego_degrad = Image.fromarray(noisy)
            else:  # Gaussian blur
                sigma_blur = 2
                image_stego_degrad = cv2.GaussianBlur(cv2.cvtColor(np.array(image_stego), cv2.COLOR_RGB2BGR), (7, 7), sigma_blur)
                image_stego_degrad = Image.fromarray(cv2.cvtColor(image_stego_degrad, cv2.COLOR_BGR2RGB))
            image_hide_latent = ode.image2latent(image_stego_degrad, ref_vae=args.ref_model)
            ...
            for i in range(2):
                for t in range(args.num_secret_image):
                    if args.key_type == 'noise_flip':        
                        for k in range(args.num_secret_image):
                            latent_p[k][i].append(torch.where(noise_mask_p[k] == 1, -latents_split[i][t], latents_split[i][t]))
                        
                    elif args.key_type == 'random_basis':
                        for k in range(args.num_secret_image):
                            latent = latents_split[i][t].reshape(B, -1)
            ...
            for i in range(args.num_secret_image):
                image_rec = ode.latent2image(latents_split_p[i][i], seed=args.seed)
                if j == 0:
                    cv2.imwrite("{:s}/{:s}_{:d}_rec_wo_degrad.png".format(args.save_path, image_name, i+1),
                            cv2.cvtColor(np.array((ToPILImage()(image_rec)).resize((512, 512))), cv2.COLOR_RGB2BGR))
                elif j == 1: # JPG compressison
                    cv2.imwrite("{:s}/{:s}_{:d}_rec_JPG_{:d}.png".format(args.save_path, image_name, i+1, quality),
                            cv2.cvtColor(np.array((ToPILImage()(image_rec)).resize((512, 512))), cv2.COLOR_RGB2BGR))
                elif j == 2: # Gaussian noise
                    cv2.imwrite("{:s}/{:s}_{:d}_rec_Gnoise_{:d}.png".format(args.save_path, image_name, i+1, sigma_noise),
                            cv2.cvtColor(np.array((ToPILImage()(image_rec)).resize((512, 512))), cv2.COLOR_RGB2BGR))
                else: # Gaussian blur
                    cv2.imwrite("{:s}/{:s}_{:d}_rec_Gblur_{:d}.png".format(args.save_path, image_name, i+1, sigma_blur),
                            cv2.cvtColor(np.array((ToPILImage()(image_rec)).resize((512, 512))), cv2.COLOR_RGB2BGR))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--private_strength', type=float, default=0.5, help='private period of diffusion process')
    ...
    parser.add_argument('--num_secret_image', type=int, default=2, help='number of secret images per one stego image')
